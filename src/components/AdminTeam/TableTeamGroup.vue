<template>
  <div class="w-full flex flex-col bg-gray-200">
    <div class="bg-white">
      ...
      >
        <td
          class="w-2/12 py-3 px-6 text-left flex items-center whitespace-nowrap"
        >
        ...
        <td class="w-2/12 py-3 px-6 text-center whitespace-nowrap">
          <div class="flex flex-col items-center justify-center">
            <div>
              <span class="border-b border-gray-400 select-none">Логин:</span>
              {{ agent.login }}
            </div>
            <div>
              <span class="border-b border-gray-400 select-none">Пароль:</span>
              {{ agent.password }}
            </div>
          </div>
        </td>
        ...
        <td class="w-2/12 py-3 px-6 text-center whitespace-nowrap">
          <div class="flex items-center justify-center">
            <span class="font-medium">{{
              agent.createdAt | date("fullmonthDayPlusTime")
            }}</span>
          </div>
        </td>
        ...
        <td
          v-if="!agent.isAdmin"
          class="w-1/12 py-3 px-6 text-center whitespace-nowrap"
        >
          <div class="flex items-center justify-center">
        ...
        <td
          v-if="!agent.isAdmin"
          class="w-2/12 py-3 px-6 text-left whitespace-nowrap"
        >
          <div class="flex items-center justify-center">
        ...
        >
          <div class="flex items-center justify-center">
            Принятых звонков:
            <span v-if="callCount(agent.id) !== 0" class="font-medium ml-1">{{
              (
                (callNotMissedCount(agent.id) / callCount(agent.id)) *
                100
              ).toFixed(2) + "%"
            }}</span>
        ...
        <td class="w-1/12 py-3 px-6 text-center">
          <div class="flex items-center justify-center">
            ...
            <div
              class="w-5 mr-2 group cursor-pointer relative"
  ...
  methods: {
    ...
    callMissedCount(id) {
      ...
      const matched = this.clients.filter(
        (client) => client.author === id && client.missedCall
      );
      return matched.length;
    },
