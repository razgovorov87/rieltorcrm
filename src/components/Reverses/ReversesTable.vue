    ...
    <table class="w-full bg-white shadow-md rounded mt-3 mb-6">
      <thead>
        ...
        >
          ...
          <th class="py-3 px-6 text-center w-2/12 truncate">
            Ссылка на объект
          </th>
      ...
      >
        <div class="bg-white">
          ...
          >
            ...
            >
              <div class="flex items-center">
                <span class="font-medium border-b border-dashed border-gray-400"
                  >{{ item.date | date("fullmonthDayAndYear") }} в
                  {{ item.time }}</span
                >
              </div>
            </td>
              ...
              <div class="flex items-center">
                <span
                  ...
                  class="
                    ...
                    cursor-pointer
                  "
                  >{{ takeClientInfo(item.clientId) }}</span
                >
                <div v-else class="font-medium">
                  <span class="text-gray-400 border-b-0">Агент:</span>
                  {{ takeAgentPhone(item.agent) }}
                </div>
              </div>
            </td>
            ...
            >
              <div class="flex items-center justify-center">
                <div
                  v-if="item.result && item.agent === uid"
                  class="
                    font-medium
                    text-white
                    flex
                    ...
                    cursor-pointer
                  "
                  @click="$emit('openResultDialog', item)"
                >
                  <span
                    v-if="item.result === 'Оставили залог'"
                    class="px-3 py-1 bg-green-500 shadow rounded text-white"
                    >{{ item.result }}</span
                  >
                  <span
                    v-else-if="item.result === 'Клиент думает'"
                    class="px-3 py-1 bg-yellow-300 shadow rounded text-gray-700"
                    >{{ item.result }}</span
                  >
                  <span
                    v-else-if="item.result === 'Клиент отказался'"
                    class="px-3 py-1 bg-red-500 shadow rounded text-white"
                    >{{ item.result }}</span
                  >
                  <span
                    v-else-if="item.result === 'Подписали договор'"
                    class="px-3 py-1 bg-green-300 shadow rounded text-gray-700"
                    >{{ item.result }}</span
                  >
                  <span
                    v-else-if="item.result === 'Просмотр отменился'"
                    class="px-3 py-1 bg-red-500 shadow rounded text-white"
                    >{{ item.result }}</span
                  >
                ...
                <div
                  v-else-if="item.result && item.agent !== uid"
                  class="font-medium text-white flex items-center"
                >
                  <span
                    v-if="item.result === 'Оставили залог'"
                    class="px-3 py-1 bg-green-500 shadow rounded text-white"
...
export default {
  ...
  computed: {
    sortedReserve() {
      ...
      const arr = Object.keys(this.reverses).map((key) => ({
        ...this.reverses[key],
      }));
