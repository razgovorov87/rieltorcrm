  ...
  <div class="flex flex-col px-8 py-4">
    ...
    <div class="flex flex-col">
      <div
        ...
        :key="cause + idx"
        class="flex flex-col mt-4 border-2 px-3 py-2 rounded bg-white space-y-3"
      >
        <div class="flex items-center">
          <span class="w-36">Причина отказа:</span>
          <span
            v-if="cause.cause !== 'Другое'"
            class="flex-grow border-b-2 border-gray-300 border-dashed italic"
            >{{ cause.cause }}</span
          >
          <span
            v-else
            class="flex-grow border-b-2 border-gray-300 border-dashed italic"
            >{{ cause.otherCause }}</span
          >
        </div>
        ...
        <div class="flex items-center w-full">
          ...
          <span class="font-medium flex-grow">{{
            takeAgentInfo(cause.agent)
          }}</span>
        </div>

        <div class="flex items-center w-full">
          <span class="w-36">Дата:</span>
          <span class="rounded-full bg-white shadow border px-3">{{
            cause.date | date("fullmonthDayPlusTime")
          }}</span>
        </div>
      </div>
    </div>
